...
const GoalBox = styled.div`
    ...
    border-radius: 20px;
    border: 3px solid ${({ isClicked }) => isClicked ? ORIGINAL_YELLOW : 'lightgray'};
    width: 350px;
    height: 40px;
    display: flex;
    align-items: center;
...
const GoalComponent = ({ isYearButtonClicked, isMonthButtonClicked, onDelete }) => {
    ...
    return (
        <GoalComponentBox>
            {goalData.year != null ? (
                ...
                <GoalBox isClicked={isCompleted}>
                    {isCompleted ? 'C O M P L E T E ! ! !' : goalData.monthGoals[month-1]} {/* Assuming monthGoals is an object with a month property */}
                </GoalBox>
            )}
            ...
            {goalData.year != null ? ( null ) : (
                (visibleDetail &&
                    <DetailGoal>
                        {goalData &&
                            goalData.monthGoals.map((monthGoal, index) => (
                                <React.Fragment key={index}>
                                    {index !== 0 && <br />} {/* Add a line break except for the first item */}
                                    {`${index + 1}월\n${monthGoal}`}
                                </React.Fragment>
                        ))}
                    </DetailGoal>
